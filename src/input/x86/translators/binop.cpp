...
void binop_translator::do_translate() {
    ...
    case XED_ICLASS_BSF: {
        ...
        // if src == 0
        auto src_zero = builder().insert_cmpeq(
        ...
        builder().insert_write_local(
            mask, builder().insert_constant_i(type, 1)->val()); // mask := 0x1
        if (inst_class == XED_ICLASS_BSR) {
            auto mask_val = builder().insert_read_local(mask);
            mask_val = builder().insert_lsl(
                mask_val->val(),
                builder().insert_constant_i(type, type.width() - 1)->val());
            builder().insert_write_local(
                mask, mask_val->val()); // mask := mask << width - 1
            builder().insert_write_local(
                idx,
                builder()
                    .insert_constant_u64(type.width() - 1)
                    ->val()); // idx = width - 1
        } else {
            builder().insert_write_local(
                idx, builder().insert_constant_u64(0)->val()); // idx := 0
        }
        ...
        auto mask_val = builder().insert_read_local(mask);
        auto mask_and = builder().insert_cmpne(
            builder().insert_and(mask_val->val(), src->val())->val(),
            builder().insert_constant_i(type, 0)->val());
        cond_br_node *fi_br =
            (cond_br_node *)builder().insert_cond_br(mask_and->val(), nullptr);
        //       idx--/++; mask = mask >>/<< 1
        auto idx_val = builder().insert_read_local(idx);
        if (inst_class == XED_ICLASS_BSR) {
            idx_val = builder().insert_sub(
                idx_val->val(), builder().insert_constant_u64(1)->val());
            mask_val = builder().insert_lsr(
                mask_val->val(), builder().insert_constant_i(type, 1)->val());
        } else {
            idx_val = builder().insert_add(
                idx_val->val(), builder().insert_constant_u64(1)->val());
            mask_val = builder().insert_lsl(
                mask_val->val(), builder().insert_constant_i(type, 1)->val());
        }
