...
static constexpr const unsigned long flag_idx =
    static_cast<const unsigned long>(
        std::min({reg_idx::ZF, reg_idx::CF, reg_idx::OF, reg_idx::SF}));
static constexpr const unsigned long flag_off =
    static_cast<const unsigned long>(std::min(
        {reg_offsets::ZF, reg_offsets::CF, reg_offsets::OF, reg_offsets::SF}));

template <const size_t order[], typename Tuple, size_t... idxs>
auto riscv64_translation_context::reorder(Tuple tuple,
                                          std::index_sequence<idxs...>) {
    return std::make_tuple(
        allocate_register(std::get<order[idxs]>(tuple)).first.reg()...);
}

template <reg_idx... idx>
std::tuple<to_type_t<reg_idx, idx, RegisterOperand>...>
riscv64_translation_context::allocate_in_order(
    to_type_t<reg_idx, idx, const port *>... args) {
    auto constexpr order = ordered<reg_idx, idx...>::value;

    auto tuple = std::make_tuple(args...);

    auto allocations =
        reorder<order>(tuple, std::make_index_sequence<sizeof...(idx)>{});

    return {(args && !args->targets().empty())
                ? std::get<static_cast<const unsigned long>(idx) - flag_idx>(
                      allocations)
                : builder::none_reg...};
}
 ...
 * Will return the previously allocated TypedRegister or a new one with type set
 * accordingly to the given port.
 * @param p Port of the register. Can be null for temporary.
 * @param reg1 Optional. Use this register for the lower half instead of
 * allocating a new one.
 * @param reg2 Optional. Use this register for the upper half instead of
 * allocating a new one.
 * @return A pair reference to the allocated Register and a boolean indicating
 * whether the allocating was new.
 */
std::pair<TypedRegister &, bool> riscv64_translation_context::allocate_register(
    const port *p, std::optional<RegisterOperand> reg1,
    std::optional<RegisterOperand> reg2) {
    ...
    case 128: {
        ...
        auto [a, b] = treg_for_port_.emplace(std::piecewise_construct,
                                             std::forward_as_tuple(p),
                                             std::forward_as_tuple(r1, r2));
...
RegisterOperand
riscv64_translation_context::get_or_assign_mapped_register(uint32_t idx) {
    if (idx >= static_cast<unsigned long>(reg_idx::RAX) &&
        idx <= static_cast<unsigned long>(reg_idx::R15)) { // GPR
        uint32_t i = idx - 1;
        reg_written_[i] = reg_loaded_[i] =
            true; // Consider "assign" access as write
        RegisterOperand reg =
            RegisterOperand{RegisterOperand::FUNCTIONAL_BASE + i};
        if (!idxs_.empty()) {
        ...
        flag_written_[i] = flag_loaded_[i] =
            true; // Consider "assign" access as write
        RegisterOperand reg =
            RegisterOperand{RegisterOperand::FUNCTIONAL_BASE + 16 + i};
        if (!idxs_.empty()) {
            live_across_iteration_.push_front(reg.encoding());
        }
        return reg;
...
RegisterOperand
riscv64_translation_context::get_or_load_mapped_register(uint32_t idx) {
    if (idx >= static_cast<unsigned long>(reg_idx::RAX) &&
        idx <= static_cast<unsigned long>(reg_idx::R15)) { // GPR
        uint32_t i = idx - 1;
        RegisterOperand reg =
        ...
        if (!reg_loaded_[i]) {
            reg_loaded_[i] = true;
            builder_.ld(reg,
                        AddressOperand{FP, static_cast<intptr_t>(
                                               8 * idx)}); // FIXME hardcoded
        }
        if (!idxs_.empty()) {
            live_across_iteration_.push_front(reg.encoding());
        }
        ...
        if (!flag_loaded_[i]) {
            flag_loaded_[i] = true;
            builder_.lb(
                reg, AddressOperand{FP, static_cast<intptr_t>(
                                            flag_off + i)}); // FIXME hardcoded
        }
        if (!idxs_.empty()) {
            live_across_iteration_.push_front(reg.encoding());
        }
...
/**
 * Adds a NOP with a non standard encoding to the generated instructions as a
 * marker.
...
void riscv64_translation_context::begin_instruction(off_t address,
...
static const std::unordered_map<std::size_t, load_store_func_t>
    load_instructions{
        {1, &InstructionBuilder::lb},  {8, &InstructionBuilder::lb},
        {16, &InstructionBuilder::lh}, {32, &InstructionBuilder::lw},
        {64, &InstructionBuilder::ld},
    };

static const std::unordered_map<std::size_t, load_store_func_t>
    store_instructions{
        {1, &InstructionBuilder::sb},  {8, &InstructionBuilder::sb},
        {16, &InstructionBuilder::sh}, {32, &InstructionBuilder::sw},
...
std::optional<int64_t> riscv64_translation_context::get_as_int(const node *n) {
    ...
    case node_kinds::cast: {
        ...
        const std::optional<int64_t> &src_val =
            get_as_int(node.source_value().owner());
        if (src_val) {
    ...
    case node_kinds::bit_extract: {
        ...
        if (src_val) {
            ...
            return (((uint64_t)*src_val) << (64 - (from + length))) >>
                   (64 - (length));
        }
    } break;
...
std::optional<std::reference_wrapper<TypedRegister>>
riscv64_translation_context::materialise(const node *n) {
    ...
    switch (n->kind()) {
    case node_kinds::bit_shift:
        return materialise_bit_shift(
            *reinterpret_cast<const bit_shift_node *>(n));
    case node_kinds::read_reg:
        return materialise_read_reg(
    ...
    case node_kinds::cond_br:
        ...
        return std::nullopt;
    case node_kinds::binary_arith:
        return materialise_binary_arith(
            *reinterpret_cast<const binary_arith_node *>(n));
    case node_kinds::unary_arith:
        return materialise_unary_arith(
            *reinterpret_cast<const unary_arith_node *>(n));
    case node_kinds::ternary_arith:
        return materialise_ternary_arith(
            *reinterpret_cast<const ternary_arith_node *>(n));
    case node_kinds::bit_extract:
        return materialise_bit_extract(
            *reinterpret_cast<const bit_extract_node *>(n));
    case node_kinds::bit_insert:
        return materialise_bit_insert(
            *reinterpret_cast<const bit_insert_node *>(n));
    case node_kinds::cast:
        return materialise_cast(*reinterpret_cast<const cast_node *>(n));
    case node_kinds::binary_atomic:
        return materialise_binary_atomic(
            *reinterpret_cast<const binary_atomic_node *>(n));
    case node_kinds::ternary_atomic:
        return materialise_ternary_atomic(
...
TypedRegister &riscv64_translation_context::materialise_ternary_atomic(
    ...
    auto [zf, of, cf, sf] =
        allocate_in_order<reg_idx::ZF, reg_idx::OF, reg_idx::CF, reg_idx::SF>(
            &n.zero(), &n.overflow(), &n.carry(), &n.negative());
    ...
    load_reserve_type lr = n.val().type().element_width() == 32
                               ? &InstructionBuilder::lrw
                               : &InstructionBuilder::lrd;
    store_conditional_type sc = n.val().type().element_width() == 32
                                    ? &InstructionBuilder::scw
                                    : &InstructionBuilder::scd;
    load_store_func_t store = n.val().type().element_width() == 32
                                  ? &InstructionBuilder::sw
                                  : &InstructionBuilder::sd;
    ...
    case ternary_atomic_op::cmpxchg:
        ...
        case 32: {
            ...
            (builder_.*
             sc)(temp, src, addr,
                 std::memory_order_acq_rel); // Out_reg unused so use it here
            ...
            builder_.mv(
                get_or_assign_mapped_register(
                    reinterpret_cast<const read_reg_node *>(n.rhs().owner())
                        ->regidx()),
                out_reg);
...
TypedRegister &riscv64_translation_context::materialise_binary_atomic(
    ...
    case binary_atomic_op::add: {
        ...
        if (flags_needed) {
            add(builder_, val_reg, out_reg,
                src); // Actual sum for flag generation
    ...
    case binary_atomic_op::sub:
        ...
        if (flags_needed) {
            sub(builder_, val_reg, out_reg,
                src); // Actual difference for flag generation
    ...
    case binary_atomic_op::band: {
        ...
        case 64:
            ...
            if (flags_needed) {
                builder_.and_(val_reg, out_reg,
                              src); // Actual and for flag generation
            }
            break;
        case 32:
            ...
            if (flags_needed) {
                builder_.and_(val_reg, out_reg,
                              src); // Actual and for flag generation
                builder_.slli(val_reg, val_reg,
                              32);  // Get rid of higher 32 bits
            }
            break;
        default:
    ...
    case binary_atomic_op::bor: {
        ...
        case 64:
            ...
            if (flags_needed) {
                builder_.or_(val_reg, out_reg,
                             src); // Actual or for flag generation
            }
            break;
        case 32:
    ...
    case binary_atomic_op::bxor: {
        ...
        case 64:
            ...
            if (flags_needed) {
                builder_.xor_(val_reg, out_reg,
                              src); // Actual xor for flag generation
            }
            break;
        case 32:
...
TypedRegister &
riscv64_translation_context::materialise_cast(const cast_node &n) {
    TypedRegister &src_reg = *materialise(n.source_value().owner());

    bool works =
        (is_scalar_int(n.val()) ||
         ((is_int_vector(n.val(), 2, 64) || is_int_vector(n.val(), 4, 32) ||
           is_int_vector(n.val(), 16, 8) || is_int_vector(n.val(), 4, 128)) &&
          n.op() == cast_op::bitcast)) &&
        (is_gpr_or_flag(n.source_value()) ||
         ((is_i128(n.source_value()) || is_int(n.source_value(), 512) ||
           is_int_vector(n.source_value(), 4, 32) ||
           is_int_vector(n.source_value(), 16, 8) ||
    ...
    case cast_op::bitcast:
        ...
        // Just noop for now as all supported types use same layouts for same
        // width
        return src_reg;
    case cast_op::sx:
        if (is_i128(n.val())) {
            ...
            for (const node *target : n.val().targets()) {
                if (target->kind() != node_kinds::binary_arith) {
                    throw std::runtime_error(
                        "unsupported types on cast sx operation");
                }
                binary_arith_op op = ((const binary_arith_node *)target)->op();
                if (!(op == binary_arith_op::mul ||
                      op == binary_arith_op::div ||
                      op == binary_arith_op::mod)) {
                    throw std::runtime_error(
                        "unsupported types on cast sx operation");
                }
            }
    ...
    case cast_op::zx: {
        if (n.val().type().element_width() == 128) {
            if (n.source_value().type().element_width() == 64) {
                auto [out_reg, valid] =
                    allocate_register(&n.val(), src_reg, ZERO);
                return out_reg;
            } else {
                auto [out_reg, valid] =
                    allocate_register(&n.val(), std::nullopt, ZERO);
                if (!valid) {
                    return out_reg;
                }
                fixup(builder_, out_reg, src_reg,
                      n.val().type().get_unsigned_type());
                return out_reg;
            }
        }
...
TypedRegister &riscv64_translation_context::materialise_bit_extract(
...
TypedRegister &
riscv64_translation_context::materialise_bit_insert(const bit_insert_node &n) {
    int to = n.to();
    int length = n.length();
    ...
    if (is_i128(n.val()) && is_i128(n.source_value()) && is_gpr(n.bits())) {
        if (to == 0 && length == 64) {
            ...
            auto [out_reg, valid] =
                allocate_register(&n.val(), std::nullopt, src.reg2());
            if (valid) {
            ...
            throw std::runtime_error(
                "Unsupported bit insert arguments for 128 bit width.");
        }
    } else if (is_gpr(n.val()) && is_gpr(n.source_value()) &&
               is_gpr(n.bits())) {
...
TypedRegister &
riscv64_translation_context::materialise_read_reg(const read_reg_node &n) {
    const port &value = n.val();
    if (value.targets().size() == 1 &&
        ((is_int(value, 64) &&
          n.regidx() <= static_cast<unsigned long>(reg_idx::R15)) ||
         is_flag(value))) { // 64bit GPR or flag only used once
    ...
    if (is_gpr(value)) {
        if (n.regidx() > static_cast<unsigned long>(reg_idx::R15)) { // Not GPR
            auto load_instr =
                load_instructions.at(value.type().element_width());
            (builder_.*load_instr)(
                out_reg, AddressOperand{FP, static_cast<intptr_t>(n.regoff())});
    ...
    if (is_flag(value)) {
        ...
        builder_.ld(out_reg.reg1(),
                    AddressOperand{FP, static_cast<intptr_t>(n.regoff())});
        builder_.ld(out_reg.reg2(),
                    AddressOperand{FP, static_cast<intptr_t>(n.regoff() + 8)});
...
void riscv64_translation_context::materialise_write_reg(
    const write_reg_node &n) {
    const port &value = n.value();
    if (is_gpr(value)) {
        TypedRegister &reg = *(materialise(value.owner()));
        if (n.regidx() > static_cast<unsigned long>(reg_idx::R15) ||
            n.regidx() < static_cast<unsigned long>(reg_idx::RAX)) { // Not GPR
            auto store_instr =
                store_instructions.at(value.type().element_width());
            (builder_.*store_instr)(
                reg, AddressOperand{FP, static_cast<intptr_t>(n.regoff())});
        } else {
        ...
        RegisterOperand reg = (!is_flag_port(value))
                                  ? reg_in
                                  : reg_in.flag(n.regidx() - flag_idx);
        builder_.mv(get_or_assign_mapped_register(n.regidx()), reg);
        return;
    } else if (is_i128(value) || is_int_vector(value, 2, 64) ||
               is_int_vector(value, 4, 32) || is_int(value, 512) ||
               is_int_vector(value, 4, 128)) {
        // Treat 512 as 128 for now. Assuming it is just 128 bit instructions
        // acting on 512 registers
        TypedRegister &reg =
            *(materialise(value.owner())); // Will give lower 64bit

        builder_.sd(reg.reg1(),
                    AddressOperand{FP, static_cast<intptr_t>(n.regoff())});
        builder_.sd(reg.reg2(),
                    AddressOperand{FP, static_cast<intptr_t>(n.regoff() + 8)});
        return;
    }
...
TypedRegister &
riscv64_translation_context::materialise_read_mem(const read_mem_node &n) {
    auto [out_reg, valid] = allocate_register(&n.val());
    if (!valid) {
        return out_reg;
    }
    TypedRegister &addr_reg = *(materialise(
        n.address().owner())); // FIXME Assumes address has 64bit size in IR
...
void riscv64_translation_context::materialise_write_mem(
    const write_mem_node &n) {
...
TypedRegister &
riscv64_translation_context::materialise_read_pc(const read_pc_node &n) {
    return materialise_constant(current_address_);
}
...
void riscv64_translation_context::materialise_write_pc(const write_pc_node &n) {
    ...
    if (ret_val_ == 0) { // Only chain on normal block end
        ...
        if (target) {    // Unconditional direct jump or call
            ...
            // Save address to patch jump into when chaining (A1 second ret
            // value)
            builder_.auipc(A1, 0);

            TypedRegister &reg = materialise_constant(*target);
            builder_.sd(reg, AddressOperand{
                                 FP, static_cast<intptr_t>(reg_offsets::PC)});
            return;
        } else if (n.value().owner()->kind() == node_kinds::csel) {
            const auto &node =
                *reinterpret_cast<const csel_node *>(n.value().owner());
            const std::optional<int64_t> &target1 =
                get_as_int(node.falseval().owner());
            const std::optional<int64_t> &target2 =
                get_as_int(node.trueval().owner());
            if (target1 && target2) { // Conditional direct jump
                ...
                builder_.sd(trueval, AddressOperand{FP, static_cast<intptr_t>(
                                                            reg_offsets::PC)});
                builder_.j(end, Assembler::kNearJump);
                ...
                builder_.sd(falseval, AddressOperand{FP, static_cast<intptr_t>(
                                                             reg_offsets::PC)});
...
void riscv64_translation_context::materialise_label(const label_node &n) {
    auto [it, not_exist] =
        labels_.try_emplace(&n, std::make_pair(nullptr, true));
...
inline void riscv64_translation_context::bw_branch_vreg_helper(bool bw) {
    if (bw) {
        idxs_.pop();
        for (auto it = live_across_iteration_.cbefore_begin(),
                  it_next = std::next(it);
             it_next != live_across_iteration_.cend();) {
            const RegisterOperand elem = RegisterOperand{*it_next};
            if (idxs_.empty() ||
                (elem.encoding() >= RegisterOperand::VIRTUAL_BASE &&
                 idxs_.top() < elem.encoding())) {
...
void riscv64_translation_context::materialise_br(const br_node &n) {
    auto [it, not_exist] =
        labels_.try_emplace(n.target(), std::make_pair(nullptr, false));
...
TypedRegister &riscv64_translation_context::materialise_unary_arith(
...
TypedRegister &riscv64_translation_context::materialise_ternary_arith(
    const ternary_arith_node &n) {
    if (!(is_gpr(n.val()) && is_gpr(n.lhs()) && is_gpr(n.rhs()) &&
          is_gpr(n.top()))) {
        throw std::runtime_error(
            "unsupported width on ternary arith operation");
    }
    auto [out_reg, valid] = allocate_register(&n.val());
    if (!valid) {
        return out_reg;
    ...
    RegisterOperand tmp_carry =
        cf ? allocate_register().first : builder::none_reg;
    RegisterOperand tmp_overflow =
        of ? allocate_register().first : builder::none_reg;
    ...
    case ternary_arith_op::adc:
        ...
        addi_flags(builder_, temp_result, src_reg2, 1, builder::none_reg,
                   tmp_overflow, tmp_carry, builder::none_reg);
        add(builder_, out_reg, src_reg1, temp_result);
        add_flags(builder_, out_reg, src_reg1, temp_result, builder::none_reg,
                  of, cf, builder::none_reg);
        if (cf) {
    ...
    case ternary_arith_op::sbb:
        ...
        sub_flags(builder_, out_reg, src_reg1, temp_result, builder::none_reg,
                  of, cf, builder::none_reg);
        if (cf) {
            builder_.or_(cf, cf, tmp_carry);     // Total carry out
        }
...
TypedRegister &
riscv64_translation_context::materialise_bit_shift(const bit_shift_node &n) {
        ...
        if (i) {
            ...
            if (amt == 64) {
                if (n.op() == shift_op::lsl) {
                    return allocate_register(&n.val(), ZERO, src_reg.reg1())
                        .first;
                } else if (n.op() == shift_op::lsr) {
                    return allocate_register(&n.val(), src_reg.reg2(), ZERO)
                        .first;
                }
            }
        }
    ...
    if (!(is_scalar_int(n.val()) && is_scalar_int(n.input()) &&
          is_gpr(n.amount()))) {
    ...
    auto [zf, _1, _2, sf] =
        allocate_in_order<reg_idx::ZF, reg_idx::OF, reg_idx::CF, reg_idx::SF>(
            &n.zero(), nullptr, nullptr, &n.negative());
...
TypedRegister &riscv64_translation_context::materialise_binary_arith(
    const binary_arith_node &n) {
    bool works =
        (is_gpr_or_flag(n.val()) && is_gpr_or_flag(n.lhs()) &&
         is_gpr_or_flag(n.rhs())) ||
        ((n.op() == binary_arith_op::mul || n.op() == binary_arith_op::div ||
          n.op() == binary_arith_op::mod || n.op() == binary_arith_op::bxor) &&
         is_i128(n.val()) && is_i128(n.lhs()) && is_i128(n.rhs())) ||
        ((is_int_vector(n.val(), 4, 32)) &&
         (n.op() == binary_arith_op::add || n.op() == binary_arith_op::sub)) ||
    ...
    if (i) {
        ...
        if (imm && IsITypeImm(imm)) {
            ...
            // Binary operations preserve sign extension, so we keep the
            // effective input type (even if larger)
            case binary_arith_op::band:
...
standardPath:
    ...
    case binary_arith_op::sub:
        ...
        break;
    // Binary operations preserve sign extension, so we can keep smaller of
    // effective input types
    case binary_arith_op::band:
    ...
    case binary_arith_op::mul:
        // FIXME
        if (n.val().type().element_width() == 128 &&
            (n.lhs().owner()->kind() != node_kinds::cast ||
             n.rhs().owner()->kind() != node_kinds::cast)) {
    ...
    case binary_arith_op::cmpeq: {
        RegisterOperand tmp =
            static_cast<const RegisterOperand>(src_reg2) != ZERO ? out_reg
                                                                 : src_reg1;
        if (static_cast<const RegisterOperand>(src_reg2) != ZERO) {
            builder_.xor_(out_reg, src_reg1, src_reg2);
        }
        builder_.seqz(out_reg, tmp);
        out_reg.set_actual_width(src_reg1.actual_width() <
                                         src_reg2.actual_width()
                                     ? src_reg1.actual_width_0()
                                     : src_reg2.actual_width_0());
        out_reg.set_type(get_minimal_type(src_reg1, src_reg2));
        return out_reg;
    }
...
TypedRegister &riscv64_translation_context::materialise_constant(int64_t imm) {
    // Optimizations with left or right shift at the end not implemented (for
    // constants with trailing or leading zeroes)
...
TypedRegister &
riscv64_translation_context::materialise_csel(const csel_node &n) {
    if (!(is_gpr(n.val()) && is_gpr_or_flag(n.condition()) &&
          is_gpr(n.falseval()) && is_gpr(n.trueval()))) {
    ...
    builder_.beqz(cond,
                  false_calc); // TODO Single instruction jump optimization
    ...
    // In-types might be wider than out-type so out accurate to narrower of the
    // two
    out_reg.set_type(get_minimal_type(trueval, falseval));
    out_reg.set_actual_width(trueval.actual_width() < falseval.actual_width()
                                 ? trueval.actual_width_0()
                                 : falseval.actual_width_0());
    return out_reg;
}

void riscv64_translation_context::materialise_internal_call(
...
// FIXME 512
TypedRegister &riscv64_translation_context::materialise_vector_insert(
    ...
    if (!(n.val().type().element_width() == 64 ||
          n.val().type().element_width() == 128 ||
          n.val().type().element_width() == 32)) {
        throw std::runtime_error("Unsupported vector insert element width");
    }
    if (!(n.insert_value().type().width() == 64 ||
          n.insert_value().type().width() == 128 ||
          n.insert_value().type().width() == 32)) {
        throw std::runtime_error(
            "Unsupported vector insert insert_value width");
    }
    ...
    case 32: {
        ...
        case 3: {
            auto [out_reg, valid] =
                allocate_register(&n.val(), src.reg1(), std::nullopt);
            if (valid) {
                bit_insert(builder_, out_reg, src, insert, 32 * n.index(), 32);
            }
            return out_reg;
...
TypedRegister &riscv64_translation_context::materialise_vector_extract(
    ...
    if (!(n.source_vector().type().element_width() == 64 ||
          n.source_vector().type().element_width() == 32)) {
...
/*
         Instructions:
         If FOLLOWING instruction is NOT a branch (so we do unconditional
   control flow or we are in the trueval part of conditional): If PREVIOUS
   instruction is NOT a nop: overwrite AUIPC with J (or if it doesn't fit with
   AUIPC + JR) If it is a NOP (previous instruction was an already chained
   branch, see below): overwrite previous instruction with J (or if it doesn't
   fit with AUIPC + JR) If following instruction IS a branch (so it is falseval
   part of conditional control flow): 1a. overwrite AUIPC with that branch +
   adjusted offset to following block 1b. and the branch with a NOP if the
   instruction following the branch is a J OR an AUIPC + JR: move the J (or
   AUIPC + JR) in place of the NOP (trueval part already chained)
         2. if offset doesn't fit:
                        a) overwrite AUIPC with inverted branch + single ins
   offset b) the branch with unconditional branch to block If that still doesn't
   fit: a) overwrite AUIPC with inverted branch + single ins offset b) move the
   constant generation of trueval by 1 instruction (falseval not needed anymore,
   can be overwritten) c) overwrite the branch with AUIPC d) overwrite the
   "hole" with JR
*/
void riscv64_translation_context::chain(uint64_t chain_address,
                                        void *chain_target) {
    ...
    if (IsCInstruction(following_instr_enc)) {
        CInstruction following_instr{
            static_cast<uint16_t>(following_instr_enc)};
        if (following_instr.opcode() == C_BEQZ ||
            following_instr.opcode() == C_BNEZ) {
            chain_machine_code_writer writer =
                chain_machine_code_writer{instr_p, 12};

            Assembler ass{&writer, false, RV_GC};

            intptr_t offset = ass.offset_from_target(
                reinterpret_cast<intptr_t>(chain_target));

            if (!IsBTypeImm(offset)) {
                Label end{};

                if (following_instr.opcode() == C_BEQZ) {
                    ass.bnez(following_instr.rs1p(), &end,
                             Assembler::kNearJump);
                } else {
                    ass.beqz(following_instr.rs1p(), &end,
                             Assembler::kNearJump);
                }

                offset = ass.offset_from_target(
                    reinterpret_cast<intptr_t>(chain_target));

                if (!IsJTypeImm(offset)) {
                    // We need to "make room" for "branch" (above) + AUIPC + JR.
                    // So we still need 8 Bytes (2 full size instructions) after
                    // the branch. We already have 4 Bytes due to the AUIPC, so
                    // move everything by 4 Bytes until a terminating J/JR,
                    // possibly compressed.
                    auto instr_p_next = reinterpret_cast<uint32_t *>(
                        reinterpret_cast<uint16_t *>(instr_p + 1) + 1);
                    auto save = *instr_p_next;

                    auto offLo32 = (int32_t)offset;
                    auto offLo12 = offLo32 << (32 - 12) >>
                                   (32 - 12); // sign extend lower 12 bit
                    ...
                    for (bool skip_lower = false;; instr_p_next++) {
                        ...
                        *instr_p_next = save;
                        ...
                        if (skip_lower || IsCInstruction(save)) {
                            // Handle lower half
                            if (!skip_lower) {
                                CInstruction c_instr{
                                    static_cast<uint16_t>(save)};
                                if (c_instr.opcode() == C_J) {
                                    // Found termination point, decrease offset
                                    // by 4 and reemit
                                    intptr_t new_off = c_instr.j_imm() - 4;

                                    chain_machine_code_writer writer1 =
                                        chain_machine_code_writer{instr_p_next,
                                                                  8};
                            ...
                            if (IsCInstruction(save >> 16)) {
                                ...
                                CInstruction c_instr2{
                                    static_cast<uint16_t>(save >> 16)};
                                ...
                                // Lower half of a 32bit instruction in the
                                // upper half
                                skip_lower = true;
                                Instruction split_instr{parcel << 16 |
                                                        save >> 16};
                                ...
                                if (split_instr.opcode() == JALR) {
                                    ...
                                    chain_machine_code_writer writer1 =
                                        chain_machine_code_writer{
                                            ...
                                            8};
                                    Assembler ass1{&writer1, false, RV_GC};
                                    ass1.jr(split_instr.rs1(),
                                            split_instr.itype_imm() - 4);
                                    break;
                                }
                            }
                            ...
                            if (instr.opcode() == JAL) {
                                // Instruction should be a J
                                // Found termination point, decrease offset by 4
                                // and reemit
                                intptr_t new_off = instr.jtype_imm() - 4;
                                chain_machine_code_writer writer1 =
                                    chain_machine_code_writer{instr_p_next, 8};
                                Assembler ass1{&writer1, false, RV_GC};
                                ass1.j(new_off);
                                break;
                            }
                ...
                ass.nop(IsCBImm(
                    offset)); // AUIPC full and c_bxx half size, so replacement
                              // one full and one half instruction

                // TODO Check if next instruction is J or AUIPC + JR, move those
                // inplace of the NOP
            }
        } else {
            ...
            chain_machine_code_writer writer =
                *(instr_p - 1) == 0b10011
                    ? chain_machine_code_writer{instr_p - 1, 16}
                    : chain_machine_code_writer{instr_p, 16};
            ...
            if (!IsJTypeImm(offset)) {
                throw std::runtime_error("Chaining failed. Jump offset too big "
                                         "for single direct jump instruction.");
            }
            ass.j(offset);
        }
    } else {
        ...
        if (following_instr.opcode() == BRANCH) {
            ...
            if (!IsBTypeImm(offset)) {
                ...
                case BGE:
                    ...
                    break;
                case BLTU:
                    ass.bgeu(following_instr.rs1(), following_instr.rs2(),
                             &end);
                    break;
                case BGEU:
                    ass.bltu(following_instr.rs1(), following_instr.rs2(),
                             &end);
                    break;
                default:
                ...
                switch (following_instr.funct3()) {
                case BEQ:
                    ass.beq(following_instr.rs1(), following_instr.rs2(),
                            offset);
                    break;
                case BNE:
                    ass.bne(following_instr.rs1(), following_instr.rs2(),
                            offset);
                    break;
                case BLT:
                    ass.blt(following_instr.rs1(), following_instr.rs2(),
                            offset);
                    break;
                case BGE:
                    ass.bge(following_instr.rs1(), following_instr.rs2(),
                            offset);
                    break;
                case BLTU:
                    ass.bltu(following_instr.rs1(), following_instr.rs2(),
                ...
                ass.nop(true); // AUIPC and BXX both full size, so both
                               // replacements full size
